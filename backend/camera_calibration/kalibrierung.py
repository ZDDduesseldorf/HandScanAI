...
def create_calibration_file(image_path, file_name):
    """
    creates a file that includes all calibrationvalues: mean, standard_deviation, variance_of_laplacian, extensive, Point1-4 (x y)
...
def check_kalibration(image_path, file_name):
    ...
    if (compare_mean(metrics, k_values) and  compare_std(metrics, k_values) and compare_sharpness(metrics, k_values) and compare_extensive(rectangle, k_values) and compare_rectangle(rectangle, k_values)):
...
def read_calibrationfile(file_name):
    ...
    return: list with all values in this shape [mean, standard_devitation, variance_of_laplacian, extensive, [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]]
    """
