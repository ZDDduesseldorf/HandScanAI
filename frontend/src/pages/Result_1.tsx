...
const Result_1: React.FC = () => {
  ...
  return (
    <BodyLayout>
      Ergebnis
      <SecondaryHeading>Du bist {genderGuess ? "weiblich" : "männlich"} und {ageGuess} Jahre alt</SecondaryHeading>
      <TertiaryHeading>Dein Alter</TertiaryHeading>
      <BoxText>
        HandScan AI ist sich zu {age_confidence}% sicher, dass du zwischen {min_age} und {max_age} Jahre alt bist
      </BoxText>
      <Slider disabled defaultValue={ageGuess} aria-label="Age guess" track={false} marks={age_marks} min={15} max={85} />
      <TertiaryHeading>Dein Geschlecht</TertiaryHeading>
      <BoxText>
        HandScan AI ist sich zu {gender_confidence}% sicher, dass du {genderGuess ? "weiblich" : "männlich"} bist
      </BoxText>
      <Slider
        disabled
